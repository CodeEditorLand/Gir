...
pub fn needs_mangling(name:&str) -> bool { keywords().contains_key(name) }
...
pub fn use_glib_type(env:&crate::env::Env, import:&str) -> String {
	format!(
		"{}::{}",
		if env.library.is_glib_crate() { "crate" } else { "glib" },
		import
	)
}
...
pub fn use_gio_type(env:&crate::env::Env, import:&str) -> String {
	format!(
		"{}::{}",
		if env.library.is_crate("Gio") { "crate" } else { "gio" },
		import
	)
}
...
pub fn use_gtk_type(env:&crate::env::Env, import:&str) -> String {
	format!(
		"{}::{}",
		if env.library.is_crate("Gtk") { "crate" } else { "gtk" },
		import
	)
}
