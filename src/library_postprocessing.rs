...
impl Library {
	...
	fn resolve_class_structs(&mut self) {
		...
		for (gtype_struct_c_type, struct_for_tid) in structs_and_types {
			match self.type_mut(struct_for_tid) {
				...
				x => {
					unreachable!(
						"Something other than a class or interface has a \
						 class struct: {:?}",
						x
					)
				},
					...
					if let Type::Record(r) = type_struct_type {
						if r.gtype_struct_for.as_ref() != Some(name) {
								...
								r.gtype_struct_for
							{
								...
								panic!(
									"\"{}\" has glib:type-struct=\"{}\" but \
									 the corresponding record \"{}\" has no \
						...
						panic!(
							"Element with name=\"{type_struct}\" should be a \
							 record but it isn't"
						);
					}
				} else if let Some(c) = c_class_type {
					panic!(
						"\"{name}\" has no glib:type-struct but there is an \
						 element with glib:is-gtype-struct-for=\"{c}\""
					);
				}
					...
					| Type::Union(Union { name, fields, .. }) => {
						for (fid, field) in fields.iter().enumerate() {
							...
							if field_type.maybe_ref_as::<Function>().is_some() {
								// Function pointers generally don't have
								// c_type.
								continue;
							}
							...
							if let Type::FixedArray(..) = field_type {
								...
								// HACK: field c_type used only in sys mode for
	...
	fn mark_final_types(&mut self, config:&Config) {
		...
		// there are no known subtypes.
		//
		// Final types can't have any subclasses and we handle them slightly
