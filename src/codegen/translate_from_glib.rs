...
impl TranslateFromGlib for Mode {
	fn translate_from_glib_as_function(
		...
		match ConversionType::of(env, self.typ) {
			...
			Scalar => {
				match env.library.type_(self.typ) {
			...
			Option => {
				let (pre, post) = match &self.try_from_glib {
					...
					other => {
						panic!(
							"Unexpected {other:?} for ConversionType::Option"
						)
					},
				};
			...
			Result { .. } => {
				let (pre, post) = match &self.try_from_glib {
					...
					other => {
						panic!(
							"Unexpected {other:?} for ConversionType::Result"
						)
					},
				};
				(pre.to_string(), post.to_string())
				...
				match env.type_(self.typ) {
					...
					| library::Type::CArray(..) => {
						if array_length.is_some() {
							...
							(
								format!("FromGlibPtrContainer::{}", trans.0),
								trans.1,
							)
						}
					},
				...
				match self.base_tid {
					Some(tid) => {
						let rust_type = RustType::builder(env, tid)
							...
							.try_build();
						let from_glib_xxx =
							from_glib_xxx(par.lib_par.transfer, None);
						...
						if let Some(ref msg) = self.nullable_return_is_error {
							...
							(
								...
								format!(
									"{}.{}",
									from_glib_xxx.1, suffix_function
								),
							)
						}
					},
					None if self.bool_return_is_error.is_some() => {
						(
							...
							format!(
								", \"{}\")",
								self.bool_return_is_error.as_ref().unwrap()
							),
						)
					},
					None if self.nullable_return_is_error.is_some() => {
						let res = Mode::from(par)
							.translate_from_glib_as_function(env, array_length);
						if let Some(ref msg) = self.nullable_return_is_error {
							assert!(*par.lib_par.nullable);
							(
...
fn from_glib_xxx(
	...
	match (transfer, array_length) {
		...
		(Transfer::Container, None) => {
			("from_glib_container(".into(), ")".into())
		},
