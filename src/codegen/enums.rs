...
fn generate_enum(
	...
	for member in &enum_.members {
		...
		if member.status.ignored() || vals.contains(&member.value) {
			continue;
		}
...
impl FromGlib<{sys_crate_name}::{ffi_name}> for {name} {{
    {maybe_inline}unsafe fn from_glib(value: {sys_crate_name}::{ffi_name}) -> \
		 Self {{
        {assert}",
		sys_crate_name = sys_crate_name,
		name = enum_.name,
		...
		match domain {
			ErrorDomain::Quark(quark) => {
				writeln!(
					w,
					"        static QUARK: \
					 ::std::sync::OnceLock<{0}ffi::GQuark> = \
					 ::std::sync::OnceLock::new();
        let quark = *QUARK.get_or_init(|| unsafe {{
            {0}ffi::g_quark_from_static_string(b\"{1}\\0\".as_ptr() as *const \
					 _)
        }});
