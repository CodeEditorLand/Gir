...
pub fn get_must_use_if_needed(
	...
	if let Some(parent_type_id) = parent_type_id {
		// Check it's a trait declaration or a method declaration (outside of a
		...
		if analysis.kind == library::FunctionKind::Method {
			...
			// If there is only one type returned, we check if it's the same
			// type as `self` (stored in `parent_type_id`).
...
pub fn body_chunk_futures(
	...
	if async_future.is_method {
		writeln!(
			body,
			"Box_::pin({gio_future_name}::new(self, move |obj, cancellable, \
			 send| {{"
		)?;
	} else {
		writeln!(
			body,
			"Box_::pin({gio_future_name}::new(&(), move |_obj, cancellable, \
			 send| {{"
		)?;
	}
