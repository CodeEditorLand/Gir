...
pub fn uses(
	...
	let mut grouped_imports:BTreeMap<
		(&str, Option<ImportConditions>),
		Vec<&str>,
	> = BTreeMap::new();
...
pub fn define_object_type(
	...
	let parents:Vec<StatusedTypeId> =
		parents.iter().filter(|p| !p.status.ignored()).cloned().collect();
	...
	if parents.is_empty() {
		...
		if !parents.is_empty() {
			parents_string
				.push_str(format!(" @extends {}", parents.join(", ")).as_str());
		}
		...
		if !interfaces.is_empty() {
			...
			parents_string.push_str(
				format!(" @implements {}", interfaces.join(", ")).as_str(),
			);
		}
...
fn define_boxed_type_internal(
	...
	writeln!(w, "\tmatch fn {{")?;
	let mut_ov =
		if copy_fn.first_parameter_mut { "mut_override(ptr)" } else { "ptr" };
	writeln!(
		w,
		"\t\tcopy => |ptr| {}::{}({}),",
...
pub fn cfg_deprecated(
	...
	if let Some(s) =
		cfg_deprecated_string(env, type_tid, deprecated, commented, indent)
	{
		writeln!(w, "{s}")?;
	}
	Ok(())
...
pub fn version_condition(
	...
	if let Some(s) =
		version_condition_string(env, ns_id, version, commented, indent)
	{
		writeln!(w, "{s}")?;
	}
	Ok(())
...
pub fn version_condition_doc(
	...
	match version {
		Some(v) if v > env.config.min_cfg_version => {
			if let Some(s) = cfg_condition_string_doc(
				Some(&v.to_cfg(None)),
				commented,
				indent,
			) {
...
pub fn cfg_condition_no_doc(
	...
	if let Some(s) =
		cfg_condition_string_no_doc(cfg_condition, commented, indent)
	{
		writeln!(w, "{s}")?;
	}
	Ok(())
