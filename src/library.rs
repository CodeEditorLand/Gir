...
impl Record {
	...
	pub fn has_copy(&self) -> bool {
		self.functions.iter().any(|f| f.name == "copy" || f.name == "copy_into")
	}
...
impl Type {
	pub fn get_name(&self) -> String {
		match self {
			...
			Self::FixedArray(type_id, size, _) => {
				format!("FixedArray {type_id:?}; {size}")
			},
				...
				|| x.is_bitfield()
			{
					...
					&& depr_version >= env.config.min_cfg_version
				{
					...
					if let Some(version) = version {
						println!(
							"[NOT GENERATED] {full_name} (deprecated in \
							 {version})"
						);
					} else {
		...
		for func in functions {
			...
			if !errors.is_empty() {
				...
				if errors.len() > 1 {
					...
					println!(
						"[NOT GENERATED {kind}] \
						 {full_name}{deprecated_version} because of {begin} \
						 and {end}"
					);
				} else {
					println!(
