...
pub fn read_external_libraries(
	toml:&toml::Value,
) -> Result<Vec<ExternalLibrary>, String> {
	let mut external_libraries = match toml.lookup("options.external_libraries")
	{
		Some(a) => {
			a.as_result_vec("options.external_libraries")?
				.iter()
	...
	let custom_libs =
		toml.lookup("external_libraries").and_then(toml::Value::as_table);
		...
		for custom_lib in custom_libs {
			if let Some(info) = custom_lib.1.as_table() {
				...
				let crate_name_ = info.get("crate").map_or_else(
					...
					|c| {
						c.as_str()
							.expect("crate name must be a string")
							.to_string()
					},
				);
				let min_version = info
					...
					.map(|v| {
						v.as_str()
							.expect("min required version must be a string")
					})
					.map(|v| {
						Version::from_str(v).expect("Invalid version number")
					});
				...
				let lib = ExternalLibrary {
					...
					crate_name:crate_name_.clone(),
					lib_name:crate_name(
						custom_lib.1.as_str().expect("No custom lib name set"),
					),
					min_version:None,
				};
				external_libraries.push(lib);
