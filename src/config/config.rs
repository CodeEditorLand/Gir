...
fn test_normalize_path() {
	...
	assert_eq!(normalize_path("foo/../bar/baz/../qux").as_os_str(), "bar/qux");
	assert_eq!(normalize_path("foo/bar/baz/../../qux").as_os_str(), "foo/qux");
	assert_eq!(normalize_path("/foo/../bar").as_os_str(), "/bar");
...
impl GirVersion {
	fn new(gir_dir:impl AsRef<Path>) -> Self {
		...
		let is_submodule =
			toplevel(&gir_dir) != Path::new(".").canonicalize().ok();
		Self {
			hash:is_submodule.then(|| {
				repo_hash(&gir_dir).unwrap_or_else(|| "???".to_string())
			}),
	...
	pub fn get_hash(&self) -> Option<&str> { self.hash.as_deref() }

	pub fn get_repository_url(&self) -> Option<&str> { self.url.as_deref() }
}
...
impl Config {
	...
	where
		...
		let work_mode = match work_mode.into() {
			...
			None => {
				let s = match toml
					.lookup_str("options.work_mode", "No options.work_mode")
				{
					Ok(s) => s,
					Err(e) => {
						return Err(format!(
		...
		if girs_dirs.is_empty() {
			let dirs = toml.lookup_vec(
				"options.girs_directories",
				"No options.girs_directories",
			)?;
			for dir in dirs {
				let dir = dir.as_str().ok_or_else(|| {
					"options.girs_dirs expected to be array of string"
						.to_string()
				})?;
				girs_dirs.push(config_dir.join(dir));
			}
		}
		let mut girs_version =
			girs_dirs.iter().map(GirVersion::new).collect::<Vec<_>>();
		girs_version.sort_by(|a, b| a.gir_dir.partial_cmp(&b.gir_dir).unwrap());

		let (library_name, library_version) =
			match (library_name.into(), library_version.into()) {
				(Some(""), Some("")) | (None, None) => {
					(
						toml.lookup_str(
							"options.library",
				...
				| (None, Some(_))
				| (Some(_), None) => {
					return Err("Library and version can not be specified \
					            separately"
						.to_owned());
				},
		...
		let auto_path = match toml.lookup("options.auto_path") {
			...
			None if work_mode == WorkMode::Normal => {
				target_path.join("src").join("auto")
			},
		...
		let doc_target_path:PathBuf = match doc_target_path.into() {
			Some("") | None => {
				match toml.lookup("options.doc_target_path") {
					Some(p) => {
						config_dir
							.join(p.as_result_str("options.doc_target_path")?)
					},
		...
		let trust_return_value_nullability =
			match toml.lookup("options.trust_return_value_nullability") {
				Some(v) => {
					v.as_result_bool("options.trust_return_value_nullability")?
				},
				None => false,
			};
		...
		let generate_safety_asserts = match toml
			.lookup("options.generate_safety_asserts")
		{
			Some(v) => v.as_result_bool("options.generate_safety_asserts")?,
			None => false,
		};

		let deprecate_by_min_version = match toml
			.lookup("options.deprecate_by_min_version")
		{
			Some(v) => v.as_result_bool("options.deprecate_by_min_version")?,
			None => false,
		};

		let single_version_file =
			match toml.lookup("options.single_version_file") {
				Some(v) => {
					match v.as_result_bool("options.single_version_file") {
						Ok(false) => None,
						Ok(true) => {
							Some(make_single_version_file(None, &target_path))
						},
						Err(_) => {
							match v.as_str() {
								Some(p) => {
									Some(make_single_version_file(
										Some(p),
										&target_path,
									))
								},
								None => {
									return Err("single_version_file must be \
									            bool or string path"
										.into());
								},
							}
	...
	pub fn filter_version(&self, version:Option<Version>) -> Option<Version> {
		version.and_then(
			|v| {
				if v > self.min_cfg_version { Some(v) } else { None }
			},
		)
	}

	pub fn find_ext_library(
		&self,
		namespace:&Namespace,
	) -> Option<&ExternalLibrary> {
		self.external_libraries
			.iter()
			.find(|lib| lib.crate_name == namespace.crate_name)
	}

	pub fn min_required_version(
		&self,
		env:&Env,
		ns_id:Option<NsId>,
	) -> Option<Version> {
		...
		if ns_id == namespaces::MAIN {
			...
			self.find_ext_library(namespace).and_then(|lib| lib.min_version)
		}
	}
...
fn read_toml<P:AsRef<Path>>(filename:P) -> Result<toml::Value, String> {
	...
	let input = fs::read(&filename).map_err(|e| {
		format!("Failed to read file \"{:?}\": {}", filename.as_ref(), e)
	})?;

	let input = String::from_utf8(input).map_err(|e| {
		format!("File is not valid UTF-8 \"{:?}\": {}", filename.as_ref(), e)
	})?;
...
fn read_crate_name_overrides(toml:&toml::Value) -> HashMap<String, String> {
	...
	if let Some(a) =
		toml.lookup("crate_name_overrides").and_then(toml::Value::as_table)
	{
		for (key, value) in a {
...
fn read_extra_versions(toml:&toml::Value) -> Result<Vec<Version>, String> {
	match toml.lookup("options.extra_versions") {
		Some(a) => {
			a.as_result_vec("options.extra_versions")?
				.iter()
				.map(|v| {
					v.as_str().ok_or_else(|| {
						"options.extra_versions expected to be array of string"
							.to_string()
					})
				})
...
fn read_lib_version_overrides(
	toml:&toml::Value,
) -> Result<HashMap<Version, Version>, String> {
	...
	for o in v {
		...
		let lib = o
			.lookup_str(
				"lib_version",
				"No lib_version in lib_version_overrides",
			)?
			.parse()?;
		map.insert(cfg, lib);
	}
...
fn read_feature_dependencies(
	toml:&toml::Value,
) -> Result<HashMap<Version, Vec<String>>, String> {
	...
	for o in v {
		...
		let dependencies:Result<Vec<String>, String> = o
			.lookup_vec(
				"dependencies",
				"No dependencies in feature_dependencies",
			)?
			.iter()
			.map(|v| {
				v.as_str()
					.ok_or_else(|| {
						"feature_dependencies.dependencies expected to be \
						 array of string"
							.to_string()
					})
					.map(str::to_owned)
...
fn read_external_docs_url(toml:&toml::Value) -> Result<Option<String>, String> {
	Ok(if let Some(value) = toml.lookup("options.external_docs_url") {
