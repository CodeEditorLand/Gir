...
impl Parse for CallbackParameter {
	fn parse(toml:&Value, object_name:&str) -> Option<Self> {
		let Some(ident) = Ident::parse(toml, object_name, "callback parameter")
		else {
			error!(
...
impl Parse for Parameter {
	fn parse(toml:&Value, object_name:&str) -> Option<Self> {
		let Some(ident) = Ident::parse(toml, object_name, "function parameter")
		else {
			error!(
				"No 'name' or 'pattern' given for parameter for object {}",
				object_name
		...
		let constant =
			toml.lookup("const").and_then(Value::as_bool).unwrap_or(false);
		let move_ = toml.lookup("move").and_then(Value::as_bool);
		let nullable =
			toml.lookup("nullable").and_then(Value::as_bool).map(Nullable);
		let mandatory =
			toml.lookup("mandatory").and_then(Value::as_bool).map(Mandatory);
		let infallible =
			toml.lookup("infallible").and_then(Value::as_bool).map(Infallible);
		let length_of = toml
			.lookup("length_of")
		...
		let callback_parameters = CallbackParameters::parse(
			toml.lookup("callback_parameter"),
			object_name,
		);
...
impl Return {
	pub fn parse(toml:Option<&Value>, object_name:&str) -> Self {
		...
		let nullable =
			v.lookup("nullable").and_then(Value::as_bool).map(Nullable);
		let mandatory =
			v.lookup("mandatory").and_then(Value::as_bool).map(Mandatory);
		let infallible =
			v.lookup("infallible").and_then(Value::as_bool).map(Infallible);
		...
		let nullable_return_is_error = v
			...
			.and_then(Value::as_str)
			.map(ToOwned::to_owned);
		let use_return_for_result =
			v.lookup("use_return_for_result").and_then(Value::as_bool);
		let string_type = v.lookup("string_type").and_then(Value::as_str);
		let string_type = match string_type {
			None => None,
			Some(v) => {
				match StringType::from_str(v) {
					Ok(v) => Some(v),
					Err(error_str) => {
		...
		let type_name =
			v.lookup("type").and_then(Value::as_str).map(ToOwned::to_owned);
		if string_type.is_some() && type_name.is_some() {
			error!(
				"\"string_type\" and \"type\" parameters can't be passed at \
				 the same time for object {}, only \"type\" will be applied \
				 in this case",
				object_name
			);
		}
	...
	if let Some(rename) = rename {
		for c in &["\t", "\n", " "] {
			if rename.contains(c) {
				error!(
					"Invalid 'rename' value given to {}::{}: forbidden \
					 character '{:?}'",
...
impl Parse for Function {
	fn parse(toml:&Value, object_name:&str) -> Option<Self> {
		...
		let doc_ignore_parameters = toml
			.lookup_vec(
				"doc_ignore_parameters",
				"Invalid doc_ignore_parameters",
			)
			.map(|v| {
				v.iter().filter_map(|v| v.as_str().map(String::from)).collect()
			})
			.unwrap_or_default();
		...
		let doc_struct_name = toml
			...
			.map(ToOwned::to_owned);
		let no_future =
			toml.lookup("no_future").and_then(Value::as_bool).unwrap_or(false);
		let unsafe_ =
			toml.lookup("unsafe").and_then(Value::as_bool).unwrap_or(false);
		let rename = toml
			.lookup("rename")
			.and_then(Value::as_str)
			.map(ToOwned::to_owned);
		...
		let bypass_auto_rename = toml
			...
			.unwrap_or(false);
		let is_constructor =
			toml.lookup("constructor").and_then(Value::as_bool);
		let assertion = toml
			.lookup("assertion")
			.and_then(Value::as_str)
