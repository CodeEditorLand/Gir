...
fn parse_object(
	...
	{
		panic!(
			"`init_function_expression`, `copy_into_function_expression` and \
			 `clear_function_expression` all have to be provided or neither"
		);
	}
			...
			|| clear_function_expression.is_some())
	{
		panic!(
			"`init_function_expression`, `copy_into_function_expression` and \
			 `clear_function_expression` can only be provided for BoxedInline \
			 types"
		);
	}
	...
	if generate_trait.is_some() {
		warn!(
			"`trait` configuration is deprecated and replaced by `final_type` \
			 for object {}",
			name
		);
	}
	...
	if let Some(a) = toml.lookup(&option_name).map(|a| a.as_array().unwrap()) {
		for name in a.iter().map(|s| s.as_str().unwrap()) {
			match objects.get(name) {
				...
				Some(_) => {
					panic!("Bad name in {option_name}: {name} already defined")
				},
			}
		}
	}
			...
			&& object.status != GStatus::Ignore
		{
			...
			if let Some(type_id) = type_id {
				if library.type_(type_id).is_abstract() {
					warn!(
						"Cannot generate builder for `{}` because it's a base \
						 class",
						name
					);
					// We set this to `false` to avoid having the "not_bound"
					// mode saying that this builder should be generated.
