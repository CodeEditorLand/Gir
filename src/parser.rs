...
pub fn is_empty_c_type(c_type:&str) -> bool { c_type == EMPTY_CTYPE }
...
impl Library {
	...
	fn read_namespace(
...

		{
			...
			if let Some(s) = elem.attr("shared-library") {
				ns.shared_library = s
					.split(',')
					.filter_map(|x| {
						if !x.is_empty() { Some(String::from(x)) } else { None }
					})
					.collect();
			}
		...
		parser.elements(|parser, elem| {
			...
			match elem.name() {
				...
				"boxed" | "function-macro" | "docsection" => {
	...
	fn read_class(
		...
		let symbol_prefix =
			elem.attr_required("symbol-prefix").map(ToOwned::to_owned)?;
		...
		let deprecated_version =
			self.read_deprecated_version(parser, ns_id, elem)?;
		...
		parser.elements(|parser, elem| {
			match elem.name() {
				...
				"signal" => {
					self.read_signal(parser, ns_id, elem)
						.map(|s| signals.push(s))
				},
				"property" => {
					self.read_property(parser, ns_id, elem, &symbol_prefix).map(
						|p| {
							if let Some(p) = p {
								properties.push(p);
							}
				...
				"virtual-method" => {
					self.read_virtual_method(parser, ns_id, elem)
						.map(|v| vfns.push(v))
				},
				"doc" => parser.text().map(|t| doc = Some(t)),
				"doc-deprecated" => {
					parser.text().map(|t| doc_deprecated = Some(t))
				},
				...
				"union" => {
					...
					.map(|mut u| {
						let field_name =
							if let Some(field_name) = elem.attr("name") {
								field_name.into()
							} else {
		...
		let parent =
			elem.attr("parent").map(|s| self.find_or_stub_type(ns_id, s));
	...
	fn read_virtual_method(
		...
		let deprecated_version =
			self.read_deprecated_version(parser, ns_id, elem)?;
		let c_identifier =
			elem.attr("identifier").or_else(|| elem.attr("name"));
		...
		parser.elements(|parser, elem| {
			match elem.name() {
				"parameters" => {
					self.read_parameters(parser, ns_id, true, true)
						.map(|mut ps| params.append(&mut ps))
				},
				"return-value" => {
					if ret.is_some() {
						return Err(
							parser.fail("Too many <return-value> elements")
						);
					}
	...
	fn read_bitfield(
		...
		parser.elements(|parser, elem| {
			match elem.name() {
				"member" => {
					self.read_member(parser, ns_id, elem)
			...
			match elem.name() {
				"type" | "array" => {
					...
					let (typ, c_type, array_length) =
						self.read_type(parser, ns_id, elem)?;
			...
			match elem.name() {
				...
				"type" | "array" => {
					...
					if let Some(c_type) = c_type {
						...
						return Err(
							parser.fail("Missing <alias> target's c:type")
						);
					}
					Ok(())
				},
	...
	fn read_function(
		...
		let c_identifier =
			elem.attr("identifier").or_else(|| elem.attr("type"));
		let kind = FunctionKind::from_str(kind_str)
			.map_err(|why| parser.fail(&why))?;
		...
		parser.elements(|parser, elem| {
			match elem.name() {
				"parameters" => {
					self.read_parameters(parser, ns_id, false, is_method)
						.map(|mut ps| params.append(&mut ps))
				},
				"return-value" => {
					if ret.is_some() {
					...
					ret = Some(self.read_parameter(
						parser, ns_id, elem, false, is_method,
					)?);
					Ok(())
				},
				"doc" => parser.text().map(|t| doc = Some(t)),
		...
		let get_property = gtk_get_property
			.or(elem.attr("get-property").map(ToString::to_string));
		let set_property = gtk_set_property
			.or(elem.attr("set-property").map(ToString::to_string));
		// The last argument of a callback is ALWAYS user data, so it has to be
		// marked as such in case it's missing.
		if is_callback
			&& params.last().map(|x| x.closure.is_none()).unwrap_or(false)
		{
	...
	fn read_function_to_vec(
		...
		if let Some(f) = self.read_function_if_not_moved(
			parser,
			ns_id,
			elem.name(),
			elem,
	...
	fn read_function_if_not_moved(
		...
		self.read_function(parser, ns_id, kind_str, elem, is_callback).and_then(
			|f| {
	...
	fn read_signal(
		...
		parser.elements(|parser, elem| {
			match elem.name() {
				"parameters" => {
					self.read_parameters(parser, ns_id, true, false)
						.map(|mut ps| params.append(&mut ps))
				},
				"return-value" => {
					if ret.is_some() {
	...
	fn read_parameter(
		...
		let transfer =
			elem.attr_from_str("transfer-ownership")?.unwrap_or(Transfer::None);
		let nullable = elem.attr_bool("nullable", false);
		let scope =
			elem.attr_from_str("scope")?.unwrap_or(ParameterScope::None);
			...
			match elem.name() {
				"type" | "array" => {
					...
					if let Some((tid, None, _)) = typ {
						if allow_no_ctype {
							typ =
								Some((tid, Some(EMPTY_CTYPE.to_owned()), None));
						} else {
							return Err(parser.fail_with_position(
								&format!(
	...
	fn read_property(
		...
		let transfer = Transfer::from_str(
			elem.attr("transfer-ownership").unwrap_or("none"),
		)
		.map_err(|why| parser.fail_with_position(&why, elem.position()))?;
		...
		let setter =
			gtk_setter.or(elem.attr("setter").map(ToString::to_string));
	...
	fn read_type(
		...
		let type_name =
			elem.attr("name")
				.or_else(|| {
					if elem.name() == "array" { Some("array") } else { None }
				})
				.ok_or_else(|| {
		...
		let inner = parser.elements(|parser, elem| {
			match elem.name() {
		...
		if inner.is_empty() || type_name == "GLib.ByteArray" {
			if type_name == "array" {
				...
				&& c_type.as_deref() == Some("_Bool")
			{
				Ok((
					self.find_or_stub_type(ns_id, "bool"),
					c_type,
					array_length,
				))
			} else {
	...
	fn read_version_attribute(
		...
		if let Some(v) = elem.attr(attr) {
			match v.parse() {
				...
				Err(e) => {
					Err(parser
						.fail(&format!("Invalid `{attr}` attribute: {e}")))
				},
			}
		} else {
			Ok(None)
