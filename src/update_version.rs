...
pub fn check_function_real_version(library:&mut Library) {
	...
	for typ in &mut library.namespace_mut(MAIN_NAMESPACE).types {
		match typ {
			Some(Type::Class(c)) => {
				update_function_version(&mut c.functions, library2)
			},
			Some(Type::Interface(i)) => {
				update_function_version(&mut i.functions, library2)
			},
			Some(Type::Union(u)) => {
				update_function_version(&mut u.functions, library2)
			},
			Some(Type::Record(r)) => {
				update_function_version(&mut r.functions, library2)
			},
			Some(Type::Bitfield(b)) => {
				update_function_version(&mut b.functions, library2)
			},
...
fn check_versions(
	...
	let new_version = match (*current_version, ty_version) {
		(Some(current_version), Some(ty_version)) => {
			if current_version < ty_version { Some(ty_version) } else { None }
		},
