...
use crate::{
	analysis::{
		...
		imports::Imports,
		namespaces,
		override_string_type::override_string_type_return,
		rust_type::RustType,
	},
	config,
	env::Env,
...
pub fn analyze(
	...
	let typ = configured_functions
		...
		.unwrap_or_else(|| {
			override_string_type_return(env, func.ret.typ, configured_functions)
		});
	let mut parameter = if typ == Default::default() {
		...
		Some(library::Parameter { typ, nullable, ..func.ret.clone() })
	};
	...
	let bool_return_error_message = bool_return_is_error.and_then(|m| {
		if typ != TypeId::tid_bool() && typ != TypeId::tid_c_bool() {
			error!(
				"Ignoring bool_return_is_error configuration for non-bool \
				 returning function {}",
				func.name
			);
			None
	...
	let nullable_return_error_message =
		nullable_return_is_error.and_then(|m| {
			if let Some(library::Parameter {
				nullable: Nullable(false), ..
			}) = parameter
			{
				error!(
					"Ignoring nullable_return_is_error configuration for \
					 non-none returning function {}",
					func.name
				);
				None
			} else {
	...
	let parameter = parameter.as_ref().map(|lib_par| {
		let par = analysis::Parameter::from_return_value(
			env,
			lib_par,
