...
impl IsPtr for Alias {
	fn is_ptr(&self) -> bool { self.target_c_type.contains('*') }
}
...
impl IsIncomplete for Function {
	fn is_incomplete(&self, lib:&Library) -> bool {
		// Checking p.typ.is_incomplete(lib) cause recursive check on
		// GScannerMsgFunc
...
impl IsExternal for Class {
	fn is_external(&self, _lib:&Library) -> bool { self.fields.is_empty() }
}
...
impl IsExternal for Alias {
	fn is_external(&self, lib:&Library) -> bool {
		if self.is_ptr() { false } else { lib.type_(self.typ).is_external(lib) }
	}
}
	...
	fn implements_debug(&self, lib:&Library) -> bool {
		match *lib.type_(self.typ) {
			Type::FixedArray(_, size, _) => {
				size <= RUST_DERIVE_ARRAY_SIZE_LIMIT
			},
