...
impl Parameter {
	pub fn from_parameter(
		...
		Parameter {
			...
			try_from_glib:TryFromGlib::from_parameter(
				env,
				lib_par.typ,
	...
	pub fn from_return_value(
		...
		Parameter {
			lib_par:lib_par.clone(),
			try_from_glib:TryFromGlib::from_return_value(
				env,
				lib_par.typ,
...
impl Parameters {
	...
	pub fn analyze_return(
		&mut self,
		env:&Env,
		ret:&Option<analysis::Parameter>,
	) {
		let ret_data =
			ret.as_ref().map(|r| (r.lib_par.array_length, &r.try_from_glib));

		let (ind_c, try_from_glib) = match ret_data {
			Some((Some(array_length), try_from_glib)) => {
				(array_length as usize, try_from_glib)
			},
			_ => return,
		};
...
pub fn analyze(
	...
	for (pos, par) in function_parameters.iter().enumerate() {
		...
		let configured_parameters =
			configured_functions.matched_parameters(&name);

		let c_type = par.c_type.clone();
		let typ = override_string_type_parameter(
			env,
			par.typ,
			&configured_parameters,
		);
		...
		let mut add_rust_parameter = match par.direction {
			...
			| library::ParameterDirection::InOut => true,
			library::ParameterDirection::Return => false,
			library::ParameterDirection::Out => {
				!can_as_return(env, par) && !async_func
			},
		...
		let move_ = configured_parameters
			...
			.find_map(|p| p.move_)
			.unwrap_or_else(|| {
				// FIXME: drop the condition here once we have figured out how
				// to handle the Vec<T> use case, e.g with something like
				// PtrSlice
		...
		let mut array_par = configured_parameters.iter().find_map(|cp| {
			cp.length_of.as_ref().and_then(|n| {
				function_parameters.iter().find(|fp| fp.name == *n)
			})
		});
		...
		let ref_mode = RefMode::without_unneeded_mut(
			env,
			par,
			immutable,
		...
		let nullable_override =
			configured_parameters.iter().find_map(|p| p.nullable);
		let nullable = nullable_override.unwrap_or(par.nullable);

		let try_from_glib =
			TryFromGlib::from_parameter(env, typ, &configured_parameters);
		...
		if add_rust_parameter {
			let rust_par = RustParameter { name:name.clone(), typ, ind_c };
		...
		let transformation_type = match conversion {
			...
			ConversionType::Pointer => {
				TransformationType::ToGlibPointer {
					name,
					instance_parameter:par.instance_parameter,
					transfer,
					ref_mode,
					...
					pointer_cast:if matches!(
						env.library.type_(typ),
						library::Type::CArray(_)
					) && par.c_type == "gpointer"
					{
						format!(
							" as {}",
							nameutil::use_glib_if_needed(env, "ffi::gpointer")
						)
					} else {
						Default::default()
					},
			...
			ConversionType::Unknown => {
				TransformationType::ToGlibUnknown { name }
			},
		};

		let mut transformation =
			Transformation { ind_c, ind_rust, transformation_type };
		...
		match transformation.transformation_type {
			...
			| TransformationType::ToGlibUnknown { ref name, .. } => {
				if async_func && name == callback_param_name {
					...
					transformation_type =
						Some(TransformationType::ToSome(name.clone()));
				}
			},
			TransformationType::ToGlibPointer { ref name, .. } => {
				if async_func && name == data_param_name {
					...
					// In async functions, this argument is used to send the
					// callback.
					transformation_type =
						Some(TransformationType::IntoRaw(name.clone()));
				}
			},
			_ => (),
		}
...
fn detect_length<'a>(
	...
	parameters
		.get(pos - 1)
		.and_then(|p| if has_length(env, p.typ) { Some(p) } else { None })
}
