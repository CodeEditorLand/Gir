...
impl Imports {
	...
	pub fn with_defaults(
		...
		self.defaults = ImportConditions { version, constraints };
		...
		if let Some(mut name) = self.strip_crate_name(name) {
			if name == "xlib" {
				name = if self.crate_name == "gdk_x11" {
					// Dirty little hack to allow to have correct import for
					// GDKX11.
					Cow::Borrowed("x11::xlib")
				} else {
					// gtk has a module named "xlib" which is why this hack is
					// needed too.
