...
fn is_stringify(
	...
	if let Some(ret) = func.ret.parameter.as_mut() {
		...
		if func.name == "to_string" {
			// Rename to to_str to make sure it doesn't clash with
			// ToString::to_string
			assert!(
				func.new_name.is_none(),
				"A `to_string` function can't be renamed manually. It's \
				 automatically renamed to `to_str`"
			);
			func.new_name = Some("to_str".to_owned());

			// As to not change old code behaviour, assume non-nullability
			// outside enums and flags only, and exclusively for to_string.
	...
	for (pos, func) in functions.iter_mut().enumerate() {
		if is_stringify(func, parent_type, obj) {
			...
			if returns_static_ref {
				// Override the function with a &'static (non allocating)
				// -returning string if the transfer type is none and it
