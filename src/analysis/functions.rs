...
fn analyze_callbacks(
	...
	{
		// When closure data and destroy are specified in gir, they don't take
		// into account the actual closure parameter.
					...
					if let Some((mut callback, destroy_index)) =
						...
						if let Some(destroy_index) = destroy_index {
							...
							if *user_data != callback.user_data_index {
								warn_main!(
									type_tid,
									"`{}`: Different destructors cannot share \
									 the same user data",
									func_name
								);
								*commented = true;
	...
	if !destroys.is_empty() || !callbacks.is_empty() {
		...
		*parameters = function_parameters::analyze(
		...
		warn_main!(
			type_tid,
			"`{}`: this is supposed to be a callback function but no callback \
			 was found...",
			func.name
		);
		*commented = true;
	...
	}) = callback_info
	{
		...
		*commented |= callback_type.contains("/*");
		...
		if trampoline.is_some() || async_future.is_some() {
			warn_main!(
				type_tid,
				"{}: Cannot handle callbacks and async parameters at the same \
				 time for the moment",
				func.name
			);
			*commented = true;
		...
		if par.c_type != "GDestroyNotify" {
			if let Some(user_data) = par.user_data_index {
				if user_data >= c_parameters.len() {
					warn_main!(
						type_tid,
						"function `{}` has an invalid user data index of {} \
				...
				*commented = true;
				warn_main!(
					type_tid,
					"function `{}`'s callback `{}` without associated user \
					 data",
					func_name,
					par.name
				);
			...
			if let Some(destroy_index) = par.destroy_index {
				if destroy_index >= c_parameters.len() {
					warn_main!(
						type_tid,
						"function `{}` has an invalid destroy index of {} \
						 when there are {} parameters",
						func_name,
						destroy_index,
						c_parameters.len()
				...
				if c_parameters[destroy_index].0.c_type != "GDestroyNotify" {
					*commented = true;
					warn_main!(
						type_tid,
						"function `{}`'s callback `{}` has invalid destroy \
						 callback",
						func_name,
						par.name
					);
