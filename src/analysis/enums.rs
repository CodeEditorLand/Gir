...
pub fn new(env:&Env, obj:&GObject, imports:&mut Imports) -> Option<Info> {
	...
	// Gir does not currently mark the first parameter of associated enum
	// functions
	// - that are identical to its enum type - as instance parameter since most
	// languages do not support this.
		...
		if first_param.typ == enumeration_tid {
				...
				&mut t.transformation_type
			{
				*name = "self".to_owned();
				...
				panic!(
					"Enumeration function instance param must be passed as \
					 scalar, not {:?}",
