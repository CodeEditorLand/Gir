...
impl BoundType {
	pub fn need_isa(&self) -> bool { matches!(*self, Self::IsA(_)) }

	// TODO: This is just a heuristic for now, based on what we do in codegen!
	// Theoretically the surrounding function should determine whether it needs
	// to reuse an alias (ie. to use in `call_func::<P, Q, R>`) or not.
	// In the latter case an `impl` is generated instead of a type name/alias.
	pub fn has_alias(&self) -> bool { matches!(*self, Self::NoWrapper) }
}
...
impl Bounds {
	pub fn add_for_parameter(
		...
		if !par.instance_parameter && par.direction != ParameterDirection::Out {
			if let Some(bound_type) = Bounds::type_for(env, par.typ) {
				...
				if r#async
					&& (par.name == "callback"
						|| par.name.ends_with("_callback"))
				{
					let func_name = func.c_identifier.as_ref().unwrap();
					let finish_func_name =
						if let Some(finish_func_name) = &func.finish_func {
							finish_func_name.to_string()
						} else {
							finish_function_name(func_name)
						};
					if let Some(function) =
						find_function(env, &finish_func_name)
					{
						// FIXME: This should work completely based on the
						// analysis of the finish() function but that a)
						// happens afterwards and b) is not accessible
						// from here either.
						let mut out_parameters = find_out_parameters(
							env,
							function,
						...
						if let Some(ref error) = error_type {
							type_string = format!(
								"FnOnce(Result<{parameters}, {error}>) + \
								 'static"
							);
						} else {
							type_string =
								format!("FnOnce({parameters}) + 'static");
						}
					...
					|| env.library.type_(par.typ).is_function()
				{
					...
					if let Type::Function(_) = env.library.type_(par.typ) {
						...
						let mut rust_ty = RustType::builder(env, par.typ)
							...
							.scope(par.scope)
							.concurrency(concurrency);
						if let Some(callback_parameters_config) =
							callback_parameters_config
						{
							rust_ty = rust_ty.callback_parameters_config(
								callback_parameters_config,
							);
						}
						type_string = rust_ty
	...
	pub fn type_for(env:&Env, type_id:TypeId) -> Option<BoundType> {
		...
		match env.library.type_(type_id) {
			...
			Type::Class(Class { final_type: true, .. }) => None,
			Type::Class(Class { final_type: false, .. }) => Some(IsA(None)),
	...
	pub fn get_to_glib_extra(
		...
		match bound_type {
			AsRef(_) if move_ && nullable => {
				".map(|p| p.as_ref().clone().upcast())".to_owned()
			},
	...
	pub fn iter(&self) -> Iter<'_, Bound> { self.used.iter() }

	pub fn iter_lifetimes(&self) -> Iter<'_, char> { self.lifetimes.iter() }
}
...
fn find_out_parameters(
	...
	let index_to_ignore =
		find_index_to_ignore(&function.parameters, Some(&function.ret));
	function
		.parameters
		.iter()
		.enumerate()
		...
		.map(|(_, param)| {
			// FIXME: This should work completely based on the analysis of the
			// finish() function but that a) happens afterwards and b) is not
			// accessible from here either.
			let nullable = configured_functions
				.iter()
				.find_map(|f| {
					f.parameters
