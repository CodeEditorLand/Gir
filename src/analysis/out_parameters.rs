...
use crate::{
	analysis::{
		...
		functions::is_carray_with_direct_elements,
		return_value,
		rust_type::RustType,
	},
...
impl Info {
	pub fn is_empty(&self) -> bool { self.mode == Mode::None }

	pub fn iter(&self) -> Iter<'_, analysis::Parameter> { self.params.iter() }
}
...
pub fn analyze(
	...
	let nullable_override =
		configured_functions.iter().find_map(|f| f.ret.nullable);
	if func.throws {
		let return_strategy = decide_throw_function_return_strategy(
			env,
			func_ret,
			&func.name,
		...
		|| func.ret.typ == TypeId::tid_c_bool()
	{
	...
	for lib_par in &func.parameters {
		...
		if can_as_return(env, lib_par) {
			...
			lib_par.name =
				nameutil::mangle_keywords(&lib_par.name).into_owned();
			let configured_parameters =
				configured_functions.matched_parameters(&lib_par.name);
			let mut out = analysis::Parameter::from_parameter(
				env,
				&lib_par,
				&configured_parameters,
			);

			// FIXME: temporary solution for string_type, nullable override.
			// This should completely work based on the analyzed parameters
			// instead of the library parameters.
			if let Some(c_par) =
				func_c_params.iter().find(|c_par| c_par.name == lib_par.name)
			{
	...
	{
		let mut ret = analysis::Parameter::from_return_value(
			env,
			&func.ret,
			configured_functions,
		);

		// TODO: fully switch to use analyzed returns (it add too many
		// Return<Option<>>)
...
fn decide_throw_function_return_strategy(
	...
	let typ =
		ret.parameter.as_ref().map(|par| par.lib_par.typ).unwrap_or_default();
	if env.type_(typ).eq(&Type::Basic(Basic::None)) {
		ThrowFunctionReturnStrategy::Void
	} else if use_function_return_for_result(
		env,
		typ,
		func_name,
		configured_functions,
...
pub fn use_function_return_for_result(
	...
	if let Some(use_return_for_result) = use_return_for_result {
		if typ == Default::default() {
			error!(
				"Function \"{}\": use_return_for_result set to true, but \
