...
fn get_node<'a>(
	...
	let direct_supers:Vec<TypeId> = match library.type_(tid) {
		Type::Class(Class { parent, implements, .. }) => {
			parent.iter().chain(implements.iter()).copied().collect()
		},
		Type::Interface(Interface { prerequisites, .. }) => {
			prerequisites.clone()
		},
		_ => return None,
	};
	...
	for super_ in direct_supers {
		let node = get_node(library, hier, super_)
			.expect("parent must be a class or interface");
