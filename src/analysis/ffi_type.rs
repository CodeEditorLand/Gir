...
pub fn ffi_type(env:&Env, tid:TypeId, c_type:&str) -> Result {
	...
	let res = if ptr.is_empty() {
				...
				match env.library.type_(tid) {
					Type::FixedArray(_, size, _) => {
						ffi_inner(env, c_tid, c_type).map_any(|rust_type| {
							rust_type
								.alter_type(|typ_| format!("[{typ_}; {size}]"))
						})
					},
					...
					{
						info!(
							"[c:type `gpointer` instead of `*mut {}`, fixing]",
							expected
						);
		...
		// ptr not empty
		ffi_inner(env, tid, &inner).map_any(|rust_type| {
			rust_type.alter_type(|typ_| format!("{ptr} {typ_}"))
		})
	};
...
fn ffi_inner(env:&Env, tid:TypeId, inner:&str) -> Result {
	...
	match *typ {
		...
		Type::FixedArray(inner_tid, size, _) => {
			ffi_inner(env, inner_tid, inner).map_any(|rust_type| {
