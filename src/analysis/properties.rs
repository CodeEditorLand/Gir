...
pub fn analyze(
	...
	for prop in props {
		...
		if !configured_properties.iter().all(|f| f.status.need_generate()) {
			continue;
		}
...
fn analyze_property(
	...
	let bypass_auto_rename =
		configured_properties.iter().any(|f| f.bypass_auto_rename);
	...
	if generate_set
		&& generate.contains(PropertyGenerateFlags::GET)
		&& !readable
	{
		warn!(
	...
	if generate_set
		&& generate.contains(PropertyGenerateFlags::SET)
		&& !writable
	{
		warn!(
		...
		for check_get_func_name in check_get_func_names {
			...
			if has {
					...
					|| version <= prop_version
				{
					...
					// The property is required in earlier versions than the
					// getter => we need both, but there will be a name
					// clash due to auto renaming => keep the get_property
					// name.
	...
	if writable {
		...
		if has {
				...
				|| version <= prop_version
			{
				...
				// => we need both, but there will be a name clash due to auto
				// renaming => keep the set_property name.
	...
	let getter_func = functions.iter().find(|f| {
		f.get_property.as_ref() == Some(&prop.name)
			&& Some(&f.name) == prop.getter.as_ref()
	});
	let setter_func = functions.iter().find(|f| {
		f.set_property.as_ref() == Some(&prop.name)
			&& Some(&f.name) == prop.setter.as_ref()
	});

	let has_getter = getter_func.is_some_and(|g| {
		matches!(g.status, GStatus::Generate | GStatus::Manual)
	});
	let has_setter = setter_func.is_some_and(|s| {
		matches!(s.status, GStatus::Generate | GStatus::Manual)
	});

	let getter = if readable
		&& (!has_getter || prop.version < getter_func.and_then(|g| g.version))
	{
		...
		if has_getter {
			...
			get_func_name =
				getter.new_name.as_ref().unwrap_or(&getter.name).to_string();
	...
	let setter = if writable
		&& (!has_setter || prop.version < setter_func.and_then(|s| s.version))
	{
		if let Ok(rust_type) = RustType::builder(env, prop.typ)
		...
		if has_setter {
			...
			set_func_name =
				setter.new_name.as_ref().unwrap_or(&setter.name).to_string();
	...
	let notify_signal = if notifiable {
		...
		let trampoline = trampolines::analyze(
			...
			&library::Signal {
				...
				is_action:false,
				is_detailed:false, /* well, technically this *is* an instance
				                    * of a detailed
...
fn get_func_name(prop_name:&str, is_bool_getter:bool) -> (Vec<String>, String) {
	let get_rename_res =
		getter_rules::try_rename_getter_suffix(prop_name, is_bool_getter);
...
pub fn get_property_ref_modes(
	...
	let get_out_ref_mode =
		RefMode::of(env, prop.typ, library::ParameterDirection::Return);
	let mut set_in_ref_mode =
		RefMode::of(env, prop.typ, library::ParameterDirection::In);
