...
pub fn analyze(
	...
	if signal.ret.typ != Default::default() {
		...
		if let Some(ffi_type) =
			used_ffi_type(env, signal.ret.typ, &signal.ret.c_type)
		{
			used_types.push(ffi_type);
		}

		let nullable_override =
			configured_signals.iter().find_map(|f| f.ret.nullable);
	...
	let ret =
		library::Parameter { nullable:ret_nullable, ..signal.ret.clone() };
